...
namespace Summary
{
    internal class Day08_Solution : IDaySolution
    {
        ...
        public long Part2()
        {
            ...
            (long offset, long length)[] cycles = new (long offset, long length)[startNodes.Count];

            for (int i = 0; i < startNodes.Count; i++)
            {
                cycles[i] = GetCycle(startNodes[i], instructions, dict);
            }

            long offset = cycles[0].offset;
            long length = cycles[0].length;
            for(int i = 1; i < cycles.Length ;i++)
            {
                offset = Utilities.LCM(offset, cycles[i].offset);
                length = Utilities.LCM(length, cycles[i].length);
            }
        ...
        private static (long offset, long length) GetCycle(string startNode, string instructions, Dictionary<string, string[]> dict)
        {
            var currentNode = startNode;
            var counter = 0;
            int instCounter = 0;
            Dictionary<(string nodeName, int instruction), int> foundEndNodes = new();
            while (true)
            {
                if (currentNode.EndsWith("Z"))
                {
                    if (foundEndNodes.ContainsKey((currentNode, instCounter)))
                    {
                        var foundEndNode = foundEndNodes[(currentNode, instCounter)];
                        if (foundEndNode == counter - foundEndNode)
                        {
                            return (0, foundEndNode);
                        }
                        return (foundEndNode, counter - foundEndNode);
                    }
                    else
                    {
                        foundEndNodes.Add((currentNode, instCounter), counter);
                    }
                }
                var inst = instructions[instCounter];
                var curNext = dict[currentNode];
                if (inst == 'L')
                {
                    currentNode = curNext[0];
                ...
                counter++;
                instCounter++;
                instCounter %= instructions.Length;
            }
        }
